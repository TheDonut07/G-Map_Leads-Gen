...
async function getResultLinks(page, maxResults, skip = 0) {
  const links = await page.$$eval('div[role="feed"] a.hfpxzc', (as) =>
    as.map((a) => a.href)
  );
  return links.slice(skip, skip + maxResults);
}
...
// Runs one search term end-to-end against an already-open page/context and returns its results.
// shouldStop, if given, is polled between listings; when it returns true the term ends early
// and whatever listings were already scraped are returned as-is.
// skip lets a "Continue" run pick up after listings a previous run already collected, instead
// of re-scraping the same top results.
async function scrapeQuery(page, context, query, maxResults, shouldStop = null, skip = 0) {
  ...
  await autoScrollFeed(page, skip + maxResults);
  const links = await getResultLinks(page, maxResults, skip);
  console.log(`Found ${links.length} listing(s). Visiting each...\n`);

  const results = [];

  if (shouldStop && shouldStop()) {
    console.log('Stop requested before visiting listings for this term.\n');
    return results;
  }
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (shouldStop && shouldStop()) {
      console.log(`Stop requested — stopping after ${results.length}/${links.length} listing(s) for this term.\n`);
      break;
    }
  }
